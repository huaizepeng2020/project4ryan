...
class model3_mlp(nn.Module):
    def __init__(self,feature_name):
        ...
        self.model1 = MLP(self.num_feature, [1],dropout=0.5, batchnorm=True, activation='tanh').to(self.device)
        self.model = MLP(self.num_feature, [1],dropout=0.5, batchnorm=True, activation='tanh').to(self.device)
    ...
    def train(self,data_x_train,data_y_train,data_x_valid,data_y_valid):
        ...
        for epoch_c in range(1000000):
            ...
            with torch.no_grad():
                ...
                # valid_metric=scipy.stats.pearsonr(data_y_valid.numpy(),data_y_valid_pred.numpy())[0]
...
class model4_FM(nn.Module):
    def __init__(self,feature_name):
        ...
        self.model1 = MLP(int(self.num_feature*(1+self.num_feature)), [1],dropout=0.9, batchnorm=True, activation='tanh').to(self.device)
        self.model = MLP(int(self.num_feature*(1+self.num_feature)), [1],dropout=0.9, batchnorm=True, activation='tanh').to(self.device)
        self.optimizer1 = torch.optim.Adam(self.model1.parameters(), lr=0.001)
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=0.001)
        ...
        for index, hidden_layer in enumerate(hidden_layers):
            ...
            if activation:
                if activation.lower() == 'relu':
                    ...
                    modules[f"activation{index}"] = nn.Tanh()
                else:
...
class Base_Transformer(nn.Module):
    def __init__(self, max_len,input_size):
        ...
        self.mlp = nn.Linear(int(max_len*4), 1)
        self._sequential = nn.Sequential(modules)
    ...
    def forward(self, input):
        ...
        e1=self.m1(x)
        e2=torch.tanh(self.mlp(e1.reshape(e1.shape[0],-1)))
